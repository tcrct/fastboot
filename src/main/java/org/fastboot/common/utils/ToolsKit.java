...
public class ToolsKit {
    ...
    public static Class<?> getSuperClassGenericType(final Class<?> clazz, final int index) {
        ...
        if (!(genType instanceof ParameterizedType)) {
            LogUtils.log(LOGGER, String.format("Warn: %s's superclass not ParameterizedType", clazz.getSimpleName()));
            return Object.class;
        }
        Type[] params = ((ParameterizedType) genType).getActualTypeArguments();
        if (index >= params.length || index < 0) {
            LogUtils.log(LOGGER, String.format("Warn: Index: %s, Size of %s's Parameterized Type: %s .", index,
                    clazz.getSimpleName(), params.length));
            return Object.class;
        }
        if (!(params[index] instanceof Class)) {
            LogUtils.log(LOGGER, String.format("Warn: %s not set the actual class on superclass generic parameter",
                    clazz.getSimpleName()));
            return Object.class;
        }
